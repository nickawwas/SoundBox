...
const fav = async (msg, args, playlist, musicQ) => {
  ...
  if (i === -1) return msg.channel.send(`Favorite ${fav} Doesn't Exist`);
...
const adfav = async (msg, args, playlist) => {
  ...
  let contains = false;
  playlist.forEach(({ alias }) =>
    alias.includes(fav) ? (contains = true) : false
  );

  if (contains) return msg.channel.send(`Favorite ${fav} Already Exists!`);
